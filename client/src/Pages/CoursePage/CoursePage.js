...
const CoursePage = () => {
    ...
    const fetchData = async (courseId, token) => {
      try {
        const sectionsData = await CourseServices.getAllSectionOfCourse(courseId, token);
    
        for (const section of sectionsData) {
          const lecturesBySectionId = await CourseServices.getAllLectureOfCourse(courseId, token);
 ...
 const [lectureVideoId,setLectureVideoId]= useState(getVideoIdFromUrl(sectionsNew[0].lectures[1].video_url))
...
if (
    ...
    const selectedLecture =
      sectionsNew[sectionIndex].lectures[subTitleIndex];
